...
Overall, there is indication of a difference in activity patterns between weekends and weekdays. Figure \ref{fig:eda} (both) shows that the high activity peaks are more consistent on weekdays although they do change by date, versus the high peaks on the weekends which are more variable.
...
print(xtable(q1, caption="Estimated average number of steps taken per day using an ARIMA(3, 0, 3) model on the unbinned data. The estimate, standard error and 95 percent confidence intervals (based on the t-distribution) are shown for fitting the ARIMA(3, 0, 3) model to the original data with missing observations as well as the completed data using the four prediction methods.", lab="tab1"))
@
...
In Figure \ref{fig:qtwoThree} (bottom) we can notice how this individual has two activity peaks during weekdays (7-9 am and 5-6 3pm) with a steady plateau in between them (10 am to 4pm). On weekends, this individual gets active later in the morning and keeps a rather similar activity level during the day (except for brunch time: 10 am to noon) and eventually stops being active later on the day compared to weekdays.
